...
- **Live.** A flag run goes live on its qualifying frame, a still of the flag checked like any photo: nothing ever alerts a hunter that they're near. A jailbreak goes live at least 50 m from the (public) jail. Every few seconds the phone sends a `frame`: its fix, and the SHA-256 of the video written since the last frame. The hashes, signed as they happen, pin the video: it can't be swapped afterward.
- **Challenge.** The batch that takes the streamer live draws two words, shown on confirmation and said on camera within 30 seconds. The randomness is seeded by the batch's contents and its leader's millisecond clock, so nobody, streamer included, knows or steers them early.
- **End of footage.** A flag run's footage ends 30 seconds after the challenge. A jailbreak's ends on its winning frame after 15 minutes held on camera. The stream itself can go on as long as the phone lasts: a victory lap, the player's own.
