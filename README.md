...
| Captures and jailbreaks are streamed live to the defenders and the city. Go live at least 50 m out; a frame every few seconds (fix, pose, hash of the video since the last frame); a gap over 20 s voids it. A two-word challenge appears the moment they go live, said on camera at the start; the stream must run 20 s past it. A flag run must walk up to the flag on camera. The winning frame is a still with its sensor data, checked like any photo; the referees' footage ends there, and the player can keep streaming as long as they like, a victory lap | `GameEngine.goLive`, `streamFrame`, `endStream` |
